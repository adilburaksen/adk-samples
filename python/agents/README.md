...
| [Ambient Support Agent](ambient-support-agent) | An ambient agent that processes support tickets autonomously via Pub/Sub and Cloud Scheduler triggers. Triages priority, classifies category, and alerts on-call for critical tickets via Cloud Monitoring. | Trigger endpoints, Pub/Sub, Cloud Scheduler, Cloud Monitoring, Terraform | Ambient | Easy | Single Agent | Customer Support |
