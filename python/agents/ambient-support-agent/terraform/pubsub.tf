...
#
# Messages published to the "support-tickets" topic are automatically
...
resource "google_pubsub_topic" "dead_letter" {
  name    = "support-tickets-dead-letter"
  project = var.project_id

  depends_on = [google_project_service.apis]
}

resource "google_pubsub_subscription" "support_push" {
  name    = "support-tickets-push"
  project = var.project_id
  topic   = google_pubsub_topic.support_tickets.id

  push_config {
    push_endpoint = "${data.google_cloud_run_v2_service.agent.uri}/apps/${var.agent_name}/trigger/pubsub"

    oidc_token {
      service_account_email = google_service_account.pubsub_invoker.email
      audience              = data.google_cloud_run_v2_service.agent.uri
    }
  }
